%
% function t = Worksheet5_MyAB2(f, y0, interval, Nsteps, options)
%
% Solve the ODE y' = f(x, y) on the given interval taking Nsteps with
% initial data y0.
%
function yend = Worksheet5_MyAB2(f, y0, interval, Nsteps, options)
...
for i = 2:Nsteps
    y(i+1) = y(i) + h * (3 * f(x(i), y(i)) - f(x(i-1), y(i-1))) / 2;
end
